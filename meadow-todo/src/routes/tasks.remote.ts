...
export const create = form(TaskSchema.omit({ id: true }), async (data) => {
  const { cookies } = getRequestEvent();
  const sessionToken = cookies.get("meadow.session_token");
...
export const patch = form(TaskSchema, async (data) => {
  const { cookies } = getRequestEvent();
  const sessionToken = cookies.get("meadow.session_token");
...
export const complete = command(TaskSchema, async (data) => {
  const { cookies } = getRequestEvent();
  const sessionToken = cookies.get("meadow.session_token");
